...
class Subscriber(models.Model):
    ...
    type_id = fields.Many2one('subscription.type', 'Subscription',
                              store=True)  # ,domain=[('name','=','Streaming Service')]
    ...
    def open_wizard(self):
        return {
            'type': 'ir.actions.act_window',
            'name': 'Subscription Type Wizard',
            'res_model': 'add.subscription.wizard',
            'view_mode': 'form',
            'view_type': 'form',
            'target': 'new',
    ...
    def _compute_related_records_count(self):
        for record in self:
            record.related_records_count = len(record.sub_type_ids)

    related_records_count = fields.Integer(compute='_compute_related_records_count', string='Related Records Count')
