...
class SubscriptionCloseWizard(models.TransientModel):
    ...
    def button_submit(self):
        ...
        for subscriber in self.user_id:
            ...
            sheet.write(3,1,subscriber.name)
            sheet.write(4,0, 'Age', title_format)
            sheet.write(4,1, subscriber.age)
            sheet.write(5,0, 'Birthdate', title_format)
            # sheet.write_datetime(5,1, subscriber.birthdate,date_style)
            sheet.write(3,3, 'Phone', title_format)
            sheet.write(3,4, subscriber.phone)
            sheet.write(4,3, 'Gender', title_format)
            sheet.write(4,4, subscriber.gender)
            sheet.write(5,3, 'Email', title_format)
            sheet.write(5,4, subscriber.email)
            ...
            for price in subscriber.sub_type_ids:
                sheet.write(row, 0, price.service.name)
                sheet.write(row, 1, price.plan_id.name)
